# Plan: No-Crlf README & Implementation Spec

## What This Is

Write `README.md` as a self-contained AI task baseline. The README doubles as user-facing documentation and a precise technical specification so another AI (or human) can implement the tool from it.
...
| `glob` or `regex` | Pattern matching for force rules |
...
1. **Set up dependencies** in `Cargo.toml`: clap, walkdir, ignore, toml, glob, tempfile, anyhow
2. **CLI parsing** with `clap` derive: all flags and optional directory argument
3. **Content inspector module** (`src/inspector.rs`):
   - `fn is_text(data: &[u8]) -> bool` — null byte check + printable ratio
4. **Force rules module** (`src/rules.rs`):
   - Hardcoded defaults (constant)
   - Config file loading/merging (`crlf-rules.toml`)
   - `fn determine_target(filename: &str, rules: &Rules) -> LineEnding`
   - `LineEnding` enum: `LF`, `CRLF`, `CR`
5. **Ignore module** (`src/ignore.rs`):
   - Skip `.git/` directory
   - Handle `.gitignore` via `ignore` crate (or manual parsing)
   - Merge with user-configured ignore patterns
6. **Conversion module** (`src/convert.rs`):
   - Read file content
   - Detect current line endings
   - Normalize to target
   - Atomic write (temp file + rename)
   - Forbid writing if content unchanged
7. **Main orchestration** (`src/main.rs`):
   - Walk directory
   - For each file: inspect → check rules → convert → report
   - Summary output
8. **Write README.md** with the content above
```
...
| Question | Decision |
|----------|----------|
| Text vs binary detection | Content inspection (null byte + printable ratio) |
| Force rules source | Hardcoded defaults + mergeable config file |
| Recursive by default | Yes |
| Symlinks | Skip |
| Config file name | `crlf-rules.toml` in scanned directory root |
